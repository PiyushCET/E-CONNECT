...
/*$sql="UPDATE odimob SET DIST=((((COST/DAY)-($day/28))/10)*(((COST/DAY)-($day/28))/10))+(((DATA/1024)-$data)*((DATA/1024)-$data))+(((MINS-$min)/50)*((MINS-$min)/50)) WHERE 1";*/
$sql="UPDATE odimob SET DIST=-(((COST/DAY)/($day/28))/10)+(((DATA/1024)/$data))+(((MINS/$min)/50)) WHERE 1";
mysqli_query($conn,$sql);
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE MINS<$min*0.9";
mysqli_query($conn,$sql);
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE (DATA/1024)<$data*0.9";
mysqli_query($conn,$sql);
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE (COST/DAY)>($day/28)*1.2";
mysqli_query($conn,$sql);
if($jio==0){
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE BRAND='JIO'";
mysqli_query($conn,$sql);}
if($vi==0){
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE BRAND='VI'";
mysqli_query($conn,$sql);}
if($airtel==0){
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE BRAND='AIRTEL'";
mysqli_query($conn,$sql);}
if($bsnl==0){
$sql="UPDATE odimob SET DIST=DIST+100000 WHERE BRAND='BSNL'";
mysqli_query($conn,$sql);}
$query="select * from odimob ORDER BY DIST";
$resul=mysql_query($query);
...
{
	$J=$rows['SL'];
$sql="UPDATE odimob SET VIEWS=VIEWS+5-$i WHERE SL=$J";
mysqli_query($conn,$sql);
?>

var y=<?php echo number_format(($rows['COST']/$rows['DAY'])*28); ?>;
if(y<=<?php echo $day; ?>){
ctx.fillStyle = 'rgb(200,255,200)'; 
ctx.font = 'bold 50px Arial';	
ctx.fillText ("Rs "+ y, 430, (620+(x)));}		
else{
...
ctx.fillStyle = 'rgb(200,255,200)'; 
ctx.font = 'bold 50px Arial';	
ctx.fillText (y.toFixed(3)+" GB", 860, (620+(x)));}		
else{
ctx.fillStyle = 'rgb(255,200,200)'; 
ctx.font = 'bold 50px Arial';	
ctx.fillText (y.toFixed(3)+" GB", 860, (620+(x)));	
}
var y=<?php echo $rows['MINS']; ?>;
if(y>=<?php echo $min; ?>){
	if(y==<?php echo $p; ?>){
		ctx.fillStyle = 'rgb(200,255,200)'; 
ctx.font = 'bold 30px Arial';
		ctx.fillText ("UNLIMITED MINS", 1255, 610+x);
	}
	else{
ctx.fillStyle = 'rgb(200,255,200)'; 
ctx.font = 'bold 50px Arial';	
ctx.fillText (y +" MINS", 1265, (620+(x)));}}		
else{
ctx.fillStyle = 'rgb(255,200,200)'; 
ctx.font = 'bold 50px Arial';	
ctx.fillText (y +" MINS", 1265, (620+(x)));	
...
ctx.fillText ("DAYS", 1150, 815+x);
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 40px Arial';
var y='<?php echo $rows['BRAND']; ?>';
ctx.fillText (y, 360, 1015+x);
if(<?php echo $rows['TYPE1']; ?>==1){
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 40px Arial';
ctx.fillText ("COMBO", 615, 1015+x);}
else if(<?php echo $rows['TYPE1']; ?>==2){
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 40px Arial';
ctx.fillText ("CALL", 615, 1015+x);}
else if(<?php echo $rows['TYPE1']; ?>==3){
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 40px Arial';
ctx.fillText ("DATA", 615, 1015+x);}
else{
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 35px Arial';
ctx.fillText ("VALIDITY", 615, 1015+x);}
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 40px Arial';
ctx.fillText ("Rs "+'<?php echo $rows['COST']; ?>', 880, 1015+x);
ctx.fillStyle = 'rgb(200,200,200)'; 
ctx.font = 'bold 40px Arial';
ctx.fillText ('<?php echo $rows['DAY']; ?>', 1180, 1015+x);
