...
$query="SELECT * FROM `odimob` ORDER BY VIEWS DESC";
$resul=mysql_query($query);
...
//right
ctx.fillStyle = "rgb("+(167+((i-x-u)*l))+","+(158+(i-x-u)*m)+","+(1+(i-x-u)*n)+")";
      ctx.fillRect(i,j,1,1);
}
else if(i-j>=x-y-v&&i-j<=x+u-y-v&&j<=y+v+r&&j>=y+v){
//below
ctx.fillStyle = "rgb("+(167+((j-y-v)*l))+","+(158+(j-y-v)*m)+","+(1+(j-y-v)*n)+")";
      ctx.fillRect(i,j,1,1);
}
